...
Future<void> changeTray(bool isRunning) async {
  ...
  if (isRunning) {
    //add endAlist
    items.insert(1, tm.MenuItem(key: TrayEntry.endAlist.name, label: t.tray.endAlist));
    items.insert(2, tm.MenuItem(key: TrayEntry.openGUI.name, label: t.tray.openGUI));
    ...
    //add startAlist
    items.insert(1, tm.MenuItem(key: TrayEntry.startAlist.name, label: t.tray.startAlist));
    tm.trayManager.setContextMenu(tm.Menu(items: items));
    //setToolTip() method is not available on linux,just cancel it！
    if (!Platform.isLinux) {
      await tm.trayManager.setToolTip(t.tray.tooltip);
