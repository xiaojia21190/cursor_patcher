...
class WorkingDirectoryTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final TextEditingController workingDirectoryController = TextEditingController(text: settings.workingDirectory);
                ...
                content: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    suffixIcon: IconButton(
                      ...
                      tooltip: t.settings.alistSettings.workingDirectory.chooseFrom,
                    ),
                  ),
                ),
                actions: <Widget>[
                  ...
                  FilledButton.tonal(
                    onPressed: () async {
                      final directory = Directory(workingDirectoryController.text);
                      final String programName = Platform.isWindows ? "alist.exe" : "alist";
                      try {
                        final List<FileSystemEntity> files = await directory.list().toList();
                        if (files.any((element) => element.path.endsWith(programName))) {
                          if (context.mounted) {
                            Navigator.of(context).pop(workingDirectoryController.text);
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(t.settings.alistSettings.workingDirectory.found(exec: programName)),
                              ),
                            );
                          }
                        } else {
                          workingDirectoryController.text = settings.workingDirectory;
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(t.settings.alistSettings.workingDirectory.notFound(exec: programName)),
                              ),
                            );
                          }
                        }
...
class RcloneDirectoryTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, ref) {
    ...
    final TextEditingController rcloneDirectoryController = TextEditingController(text: settings.rcloneDirectory);
            ...
            builder: (BuildContext context) {
              ...
              return AlertDialog(
                ...
                actions: <Widget>[
                  ...
                  FilledButton.tonal(
                    onPressed: () async {
                      final directory = Directory(rcloneDirectoryController.text);
                      final String programName = Platform.isWindows ? "rclone.exe" : "rclone";
                      try {
                        final List<FileSystemEntity> files = await directory.list().toList();
                        if (files.any((element) => element.path.endsWith(programName))) {
                          if (context.mounted) {
                            Navigator.of(context).pop(rcloneDirectoryController.text);
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(t.settings.alistSettings.workingDirectory.found(exec: programName)),
                              ),
                          ...
                          rcloneDirectoryController.text = settings.workingDirectory;
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(t.settings.alistSettings.workingDirectory.notFound(exec: programName)),
